...
<template>
  <div class="block sm:flex gap-0 sm:gap-10 mt-14">
    ...
    <div class="w-full sm:w-1/3 text-left">
      ...
      <div class="mb-7">
        ...
        <ul class="leading-loose">
          <li v-for="info in projectInfo.companyInfos" :key="info" class="
              font-general-regular
              text-ternary-dark
              dark:text-ternary-light
            ">
            <span>{{ info.title }}: </span>
            <a :href="info.hasOwnProperty('href') ? info.href : '#'" target="_blank" :class="info.title == 'Website' || info.title == 'Phone'
      ...
      <div class="mb-7">
        ...
        <div v-if="projectInfo.technologies[0].links">
          <!-- <p v-for="(link, index) in projectInfo.technologies[0].links" :key="index"> {{
            projectInfo.technologies[0].link
          }}</p> -->
          <p v-for="(link) in projectInfo.technologies[0].links" :key="link"
            class="font-general-regular text-blue-600 space-y-6">
            <a :href="link">{{ link }}</a>
          </p>
        </div>
      </div>
    ...
    <div class="w-full sm:w-2/3 text-left mt-10 sm:mt-0">
      ...
      <p v-for="projectDetail in projectInfo.projectDetails" :key="projectDetail.id" class="
          font-general-regular
          mb-5
